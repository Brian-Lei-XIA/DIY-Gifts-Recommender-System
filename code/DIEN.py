...
def DIEN(feature_columns, behavior_feature_list, behavior_seq_feature_list, neg_seq_feature_list, use_neg_sample=False, alpha=1.0):
    ...
    # 获取行为序列(movie_id序列, hist_item_id) 对应的embedding，这里有可能有多个行为产生了行为序列，所以需要使用列表将其放在一起
...
def get_neg_click(data_df, neg_num=10):
    movies_np = data_df['hist_item_id'].values
...
if __name__ == "__main__":
    """读取数据"""
    samples_data = pd.read_csv("../data/sample.txt", sep="\t", header = None)
    samples_data.columns = ["user_id", "gender", "age", "hist_item_id", "hist_len", "item_id", "item_type_id", "label"]

    """数据集"""
    X = samples_data[["user_id", "gender", "age", "hist_item_id", "hist_len", "item_id", "item_type_id"]]
    y = samples_data["label"]

    # 负采样，负采样的时候序列的长度和设置的行为序列长度一样长
    # 不用担心会多计算损失，其实在计算损失的时候使用mask，无效的值不会参与计算
    X['neg_hist_item_id'] = get_neg_click(X, neg_num=50)
    ...
    X_train = {"user_id": np.array(X["user_id"]), \
            ...
            "age": np.array(X["age"]), \
            "hist_item_id": np.array([[int(i) for i in l.split(',')] for l in X["hist_item_id"]]), \
            "neg_hist_item_id": np.array([[int(i) for i in l.split(',')] for l in X["neg_hist_item_id"]]), \
            "hist_len": np.array(X["hist_len"]), \
            "item_id": np.array(X["item_id"]), \
            "item_type_id": np.array(X["item_type_id"])}
    ...
    feature_columns = [SparseFeat('user_id', max(samples_data["user_id"])+1, embedding_dim=8), 
                        ...
                        SparseFeat('item_id', max(samples_data["item_id"])+1, embedding_dim=8),
                        SparseFeat('item_type_id', max(samples_data["item_type_id"])+1, embedding_dim=8),
                        DenseFeat('hist_len', 1)]

    feature_columns += [VarLenSparseFeat('hist_item_id', vocabulary_size=max(samples_data["item_id"])+1, embedding_dim=8, maxlen=50)]
    feature_columns += [VarLenSparseFeat('neg_hist_item_id', vocabulary_size=max(samples_data["item_id"])+1, embedding_dim=8, maxlen=50)]

    # 行为特征列表，表示的是基础特征
    behavior_feature_list = ['item_id']
    # 行为序列特征
    behavior_seq_feature_list = ['hist_item_id']
    # 负采样序列特征
    neg_seq_feature_list = ['neg_hist_item_id']
